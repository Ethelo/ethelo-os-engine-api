defmodule EtheloApi.Structure.Factory do
  ...
  def create_calculation_with_variables(%Decision{} = decision, overrides \\ %{}) do
    ...
    values =
      Map.get(values, :expression) ||
        Map.put(
          values,
          :expression,
          "#{values.filter_variable.slug} + #{values.detail_variable.slug}"
        )

    values =
      Map.get(values, :variables) ||
        Map.put(values, :variables, [values.filter_variable, values.detail_variable])
  ...
  def create_calculation_constraint(%Decision{} = decision, %{} = overrides) do
    ...
    constraint =
      if Map.get(values, :operator) == :between do
  ...
  def option_category_defaults() do
    %OptionCategory{
      ...
      # set this specifically as it has side effects
      quadratic: false,
  ...
  def update_option_category(%OptionCategory{id: id}, %{} = values) do
    update(OptionCategory, id, values)
  end
  ...
  def create_option_detail_value(%Decision{} = decision, %OptionDetail{} = option_detail) do
    create_option_detail_value(
      decision,
      option_detail,
      random_value_for_format(option_detail.format)
    )
  end
  ...
  def create_option_detail_filter(%Decision{} = decision, format, value)
      when is_atom(format) and is_binary(value) do
  ...
  def create_option_detail_filter(%Decision{} = decision, %OptionDetail{} = option_detail, value) do
    create_option_detail_filter(decision, :string, %{
      match_value: value,
  ...
  def create_option_detail_filter_matching(
        %Decision{} = decision,
  ...
  def create_option_category_filter_matching(
        %Decision{} = decision,
  ...
  def scenario_config_defaults() do
    %ScenarioConfig{
      ...
      ttl: Enum.random(100..10_000),
      engine_timeout: Enum.random(10_000..80_000),
