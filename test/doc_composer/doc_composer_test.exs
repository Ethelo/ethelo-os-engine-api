defmodule DocsComposer.ComposerTest do
  ...
  describe "examples" do
    ...
    def assert_valid_json(content) do
      assert %{} = Poison.Parser.parse!(content)
    end

    test "can be formatted as graphql response fragment" do
      content = DocsComposer.GraphQLFragmentBuilder.to_graphql_response_fragment(example())
      expected_strings = ["\"oneThing\": \"12\"", "\"two\": \"two words\"", "\"three\": \"an_atom\""]
      for expected <- expected_strings do
        assert content =~ ~r/#{expected}/
      end

      assert_valid_json(~s({#{content}}))
    end

    test "can be formatted as graphql search fragment" do
      content = DocsComposer.GraphQLFragmentBuilder.keys_as_graphql(example())
      expected_strings = ["oneThing", "two", "three"]
      for expected <- expected_strings do
        assert content =~ ~r/#{expected}/
      end

      excluded_strings = ["12", "two words", "an_atom"]
      for excluded <- excluded_strings do
        refute content =~ ~r/#{excluded}/
      end
    end

    test "can be formatted as graphql mutation fragment" do
      content = DocsComposer.GraphQLFragmentBuilder.to_graphql_mutation_fragment(example())
      expected_strings = ["oneThing: \"12\"", "two: \"two words\"", "three: \"an_atom\""]
      for expected <- expected_strings do
        assert content =~ ~r/#{expected}/
      end
  ...
  describe "common fields" do
    test " have all possible fields" do
      ...
      for field <- fields do
        assert %{name: _, info: _, type: _, required: _, automatic: _, immutable: _, validation: _} = field
      end
    end
  ...
  describe "schema fields" do
    ...
    def complete_field() do
      %{name: :first, info: String.duplicate("words ", 5), type: "Type", required: nil, automatic: true, immutable: true}
    end

    def complete_field_cells() do
      ["", " first ", " Type ", " words words words words words  Updated Automatically. Immutable. ", "  ", ""]
    end
