defmodule Engine.Invocation.WeightsJsonTest do
  @moduledoc false
  ...
  test "option category with no options does not error" do
    scoring_data = base_scoring_data()
      |> Map.put(:criterias, [
  ...
  test "weights on option category are normalized" do
    scoring_data = base_scoring_data()
      |> Map.put(:criterias, [
        %{id: 1, slug: "one", weighting: 50},
  ...
  test "participant weights on option category are used when toggled on" do
    scoring_data = base_scoring_data()
      |> Map.put(:criterias, [
        %{id: 1, slug: "one", weighting: 50},
      ])
    ...
    expected = [
      [0.05555555555555555, 0.16666666666666666], [0.1111111111111111, 0.16666666666666666]
    ]
