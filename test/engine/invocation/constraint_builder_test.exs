defmodule Engine.Invocation.ConstraintBuilderTest do
  @moduledoc false
  use ExUnit.Case
  ...
  test "builds constraints segments" do
    ...
    assert constraints == [
      ...
      %{code: "[filter[$Gvegetarian_no]{@total_inches * @total_inches * ( 3.14 / 4 ) / 14 - ( 0.2 * @count_cheese_yes )}] >= 2", name: "mèat_fed", relaxable: false,},
      %{code: "[@count_vegetarian_no] >= 1",name: "mèat_min", relaxable: false},
      %{code: "[@count_cheese_yes] >= 1", name: "one_cheese", relaxable: false},
      %{code: "[filter[$Gvegetarian_yes]{@total_inches * @total_inches * ( 3.14 / 4 ) / 14 - ( 0.2 * @count_cheese_yes )}] >= 2", name: "veg_fed", relaxable: false},
      %{code: "[@count_vegetarian_yes] >= 1", name: "veg_min", relaxable: false},
      %{code: "[@count_xorcat] = 1", name: "__auto_xor11", relaxable: true}
    ]

    assert displays == [
      %{code: "@total_inches * @total_inches * ( 3.14 / 4 ) / 14 - ( 0.2 * @count_cheese_yes )", name: "feeds"},
