defmodule EtheloApi.Scenarios.ScenarioHashesTest do
  @moduledoc """
  unique hashes
  """
  use EtheloApi.DataCase
  @moduletag invocation: true, data: true

  import EtheloApi.Structure.Factory
  alias EtheloApi.Invocation.ScenarioHashes
  # todo errors
  # todo group scenario hash
  # todo participant scenario hash
  ...
  def update_bin_vote(bin_vote, new_bin, decision) do
    ...
    {:ok, new_bin_vote} = EtheloApi.Voting.upsert_bin_vote(attrs, decision)
    new_bin_vote
  end
  ...
  def update_ocrv(ocrv, new_option, decision) do
    ...
    {:ok, new_ocrv} = EtheloApi.Voting.upsert_option_category_range_vote(attrs, decision)
    new_ocrv
  end
  ...
  test "Participant influent hash updates with binvotes", context do
    ...
    assert Decimal.compare(participant.weighting, Decimal.from_float(1.00)) == :eq
