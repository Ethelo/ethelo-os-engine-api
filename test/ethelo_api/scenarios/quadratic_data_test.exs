defmodule EtheloApi.Scenarios.QuadraticDataTest do
  @moduledoc """
  ...
  test "calculates" do
    ...
    import_data = ScoringData.initialize_all_voting(decision.id, scenario_config.id)
    ...
    assert 580_000 == quad_data.total_available
    assert 7500 == quad_data.cutoff
    assert 0.75 == quad_data.seed_allocation_percent
    assert 0.25 == quad_data.vote_allocation_percent
    assert 50_000 == quad_data.maximum_allocation
    assert 5000 == quad_data.round_to

    assert 435_000 == quad_data.seed_allocation_target
    assert 145_000 == quad_data.vote_allocation_target
    assert "seeds" == quad_data.seed_option_detail.slug

    # option_map = import_data.options
