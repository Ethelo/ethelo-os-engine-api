defmodule Engine.Scenarios.Docs.ScenarioConfig do
  @moduledoc "Central repository for documentation strings about ScenarioConfigs."

  require DocsComposer

  @decision_id "Unique identifier for the Decision the ScenarioConfig belongs to. All ScenarioConfigs are associated with a single Decision."
  ...
  For example, if an ScenarioConfig has 5 bins, then presentation layer could show 5 buttons:
  ...
  If enabled only the global "all options" scenario will be generated and added to the resulting scenario set.
  """
  ...
  @support_only """
  A boolean that indicates if the scenario_config should not include "disapproval" values. Defaults to "false".

  The Ethelo calculation takes into account how polarizing a Decision is - if many people are "for" or "against" an option, it will affect the score.
  ...
  For example, in a process where participants do not have relationships or the sense of reciprocity that arises in community, it may not be important that participants experience similar levels of support for an solutions, or feel that influence in shaping the solutionss was distributed fairly. However, in a stakeholder process where there are entitlements resting on a collective identity, then fairness in support distribution can be quite important. ≡ represents the importance of unity in a specific decision context.

  Collective Identity (≡) determines the impact of Unity on a decision. The higher the Collective Identity value, the more important it is that the selected solutionss have similar levels of support across participants.
  """

  @scenario_config """
  ScenarioConfigs hold the settings used when calculating solutions for a Decision.
  """

  @max_scenarios "The maximum number of solutions to generate. The actual number of solutions returned cannot be guaranteed."

  @tipping_point """
  As consensus for a solution decreases, there will be a “tipping point” when people will cease to resist the solutions because of inequality aversion, and begin to support it due to fairness and the unity it creates. Where this neutral tipping point is found will depend on the type of decision process and group dynamics.  By default, we set the tipping point at about 1/3rds consensus.
  """
  ...
  @engine_timeout "Max amount of time in millisections to allow engine to run. "

  @quadratic """
  If quadratic analysis should be run
  """

  @quad_user_seeds "Number of seeds allocated for a user in quadratic voting"
  ...
  defp scenario_config_fields() do
    [
     ...
     %{name: :bins, info: @bins, type: :integer, required: true, validation: "between 1 and 9", automatic: false, immutable: false},
     %{name: :max_scenarios, info: @max_scenarios, type: :integer, validation: "between 1 and 20", required: true, automatic: false, immutable: false},
     %{name: "ci/collective identity", info: @collective_identity, type: "decimal(8,7)", validation: "between 0 and 1", required: true, automatic: false, immutable: false},
     %{name: :tipping_point, info: @tipping_point, type: "decimal(8,7)", validation: "between 0 and 1", required: true, automatic: false, immutable: false},
     %{name: :normalize_satisfaction, info: @normalize_satisfaction, type: :boolean , required: false},
     ...
     %{name: :per_option_satisfaction, info: @per_option_satisfaction, type: :boolean , required: false},
     %{name: :solve_interval, info: @solve_interval, type: :integer , required: false},
     %{name: :ttl, info: @ttl, type: :integer , required: false},
     %{name: :engine_timeout, info: @engine_timeout, type: :integer , required: true, default: 10000},
     ...
     %{name: :quad_total_available, info: @quad_total_available, type: :integer , required: false},
  ...
  @doc """
  a list of maps describing all scenario_config schema fields

  Suitable for use with `DocsComposer.schema_fields`.
  """
  def fields() do
    DocsComposer.common_fields([:id, :slug, :inserted_at, :updated_at]) ++ scenario_config_fields()
  end
