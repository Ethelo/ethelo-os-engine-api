defmodule Engine.Scenarios.QuadraticBuilder do
  alias Engine.Scenarios.QuadraticData
  require Logger

  def calculate_totals(voting_data) do
  ...
  def build_for_output(quad_data, final_data) do
    ...
    base =  Map.take(quad_data, [
      ...
      :seeds_assigned_total, :positive_seed_votes_sum, :positive_seed_votes_sq_sum
    ])

    calculated = Map.take(final_data, [
      :seed_allocation_total, :vote_allocation_total,
      :combined_allocation_total, :final_allocation_total,
      :remaining_after_seed,
      ])

    # copy totals with name used by scenario stats
    global = Map.merge(base, calculated)
    ...
    by_oc = final_data.oc_seed_allocations
      |> Map.keys
      |> Enum.reduce(%{}, fn(oc_id, memo) ->
        oc_data = Map.get(memo, oc_id, %{})
  ...
  def calculate_seed_values(quad_data) do
    ...
    oc_seed_allocations = quad_data.seeds_assigned_by_oc
  ...
  def calculate_vote_values(quad_data, seed_data) do
    oc_vote_allocations = quad_data.positive_seed_votes_sq_by_oc
  ...
  def calculate_final_data(quad_data, combined_data) do
    oc_combined_allocations = combined_data.oc_seed_allocations
    |> Map.keys
    |> Enum.map(fn( oc_id) ->
      combined = combined_data.oc_seed_allocations[oc_id] + combined_data.oc_vote_allocations[oc_id]
      {oc_id, combined}
    end)
    |> Enum.into(%{})
    ...
    oc_final_allocations = oc_combined_allocations
    |> Enum.map(fn( {oc_id, amount} ) ->
        after_cutoff = case amount < quad_data.cutoff do
          true -> 0
