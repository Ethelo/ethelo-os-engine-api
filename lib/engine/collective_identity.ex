defmodule Engine.CollectiveIdentity do
  @moduledoc """
  ...
  def step_count(delta, step) when is_float(delta), do: step_count(Decimal.from_float(delta), step)
  ...
  def step_value(delta, step_count) when is_float(delta), do: step_value(Decimal.from_float(delta), step_count)
  def step_value(delta, step_count) when is_float(step_count), do: step_value(delta, Decimal.from_float(step_count))
  def step_value(delta, step_count) when is_integer(delta), do: step_value(Decimal.new(delta), step_count)
  def step_value(delta, step_count) when is_integer(step_count), do: step_value(delta, Decimal.new(step_count))
