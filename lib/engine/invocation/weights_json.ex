defmodule Engine.Invocation.WeightsJson do
  ...
  # option_category[0].weighting - default or overridden weight set by admin for all options in a category. 1-100, integers
  # option_category_weights - participant associated weights to apply to all options in a category 1-100, integers
  ...
  # cross referenced with the criteria to see the number of bins configured (criteria.bins) and if they are positive only or postivite to negative (criteria.support_only). Theoretically if the bin range changes all votes will be updated to correct values, but some handling should be included for out of range bins.  1-max number of bins
  ...
  def normalized_option_category_weights(voting_data) do
    ...
    |> Enum.map(fn(participant ) ->
      ...
      ocws = ( ocw_by_participant[participant.id] || []) |> group_by_option_category() |> delist_group()

      voting_data.option_categories |> Enum.map(fn(option_category ) ->
  ...
  def normalized_criteria_weights(voting_data) do
    ...
    |> Enum.map( fn participant  ->
      ...
      cws = ( cw_by_participant[participant.id] || [])
        |> group_by_criteria() |> delist_group()
  ...
  def normalized_participant_weights(voting_data) do
    weights = Enum.map(voting_data.participants, fn participant ->
  ...
  def build_normalized_weights(voting_data) do
    ...
    def calculate_influent_weight(normalized_weights, option, criteria, participant) do
      oc_weight = Map.get(normalized_weights.option_category, {option.option_category_id, participant.id}, 0.0)
  ...
  defp build_influent_weights(voting_data) do
    ...
    # each participant / option / criteria combo gets a weight
    # %{participant_id: %{criteria_id: %{option_id: weight}}}
    Enum.map( voting_data.participants, fn(participant) ->
       by_criteria = Enum.map( voting_data.criterias, fn(criteria) ->

         by_option = Enum.map( voting_data.options, fn(option) ->
            weight = calculate_influent_weight(normalized_weights, option, criteria, participant)
            {option.id, weight}
         end)
         |> Enum.into(%{})
      ...
      |> Enum.map(fn(participant_id) ->
          sorted_options |> Enum.flat_map(fn(option) ->
              sorted_criteria |> Enum.map(fn(criteria) ->
                  influent_weights |> Map.get(participant_id, %{})
                            |> Map.get(criteria.id, %{})
  ...
  def weight_list_to_average(%{} = voting_voting_data, weights, default_weight) do
    ...
    average = case weights do
         [] -> default_weight
         _ -> (Enum.sum(weights) + (with_default * default_weight))/ (length(weights) + with_default)
       end
  ...
  def compute_average_weights(%{} = voting_voting_data, %{criteria: criteria}) do
    ...
    weights = voting_voting_data.criteria_weights_by_criteria
       |> Map.get(criteria.id, [])
  ...
  def compute_average_weights(%{} = voting_voting_data, %{option_category: option_category}) do
    ...
    weights = voting_voting_data.option_category_weights_by_oc
