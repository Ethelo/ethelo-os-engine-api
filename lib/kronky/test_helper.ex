defmodule Kronky.TestHelper do
  ...
  defp assert_values_match({field, :decimal, %Decimal{} = expected, %Decimal{} = response}) do
    assert Decimal.cmp(expected, response) == :eq
  end

  defp assert_values_match({field, :decimal, expected,  response}) when is_float(response) do
    response = Decimal.from_float(response)
    assert_values_match({field, :decimal, expected, response})
  end

  defp assert_values_match({field, :decimal, expected,  response}) when is_float(expected) do
    response = Decimal.from_float(expected)
    assert_values_match({field, :decimal, expected, response})
  end

  defp assert_values_match({field, :decimal, expected,  %{"coef" => coef, "exp" => exp, "sign" => sign}}) do
    {sign, _} = Integer.parse(sign)
    {coef, _} = Integer.parse(coef)
    {exp, _} = Integer.parse(exp)
    response = Decimal.new(sign, coef, exp)
    assert_values_match({field, :decimal, expected, response})
  end

  defp assert_values_match({field, :decimal, %{"coef" => coef, "exp" => exp, "sign" => sign}, response}) do
    {sign, _} = Integer.parse(sign)
    {coef, _} = Integer.parse(coef)
    {exp, _} = Integer.parse(exp)
    expected = Decimal.new(sign, coef, exp)
    assert_values_match({field, :decimal, expected, response})
  end

  defp assert_values_match({field, :decimal, expected,  %Decimal{} = response}) do
    assert_values_match({field, :decimal, Decimal.new(expected), response})
  end

  defp assert_values_match({field, :decimal, %Decimal{} = expected, response}) do
    assert_values_match({field, :decimal, expected, Decimal.new(response)})
  end
