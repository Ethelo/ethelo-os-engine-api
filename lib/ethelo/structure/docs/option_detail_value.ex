defmodule EtheloApi.Structure.Docs.OptionDetailValue do
  ...
  @decision_id "Unique identifier for the Decision the OptionDetailValue belongs to. All OptionDetailValues are associated with a single Decision."

  @option_id "Unique identifier for the Option the OptionDetailValue belongs to."
  @option "Options are the choices that a make up a Decision solution. Your solution may include multiple Options."

  @option_detail_id "Unique identifier for the OptionDetail the OptionDetailValue belongs to."
  ...
  defp option_detail_value_fields() do
    [
     %{name: :value, info: @value, type: :string, validation: "none", required: true, automatic: false, immutable: false},
     %{name: :option_id, info: @option_id, type: "id" , required: true, validation: "must be part of the same Decision as the OptionDetail"},
     %{name: :option_detail_id, info: @option_detail_id, type: "id" , required: true, validation: "must be part of the same Decision as the Option"},
   ]
  end
  ...
  """
  def fields() do
    DocsComposer.common_fields([:inserted_at, :updated_at, :decision_id]) ++ option_detail_value_fields()
  end
