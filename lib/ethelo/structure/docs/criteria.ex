defmodule EtheloApi.Structure.Docs.Criteria do
  @moduledoc "Central repository for documentation strings about criterias."

  require DocsComposer

  @decision_id "Unique identifier for the Decision the criteria belongs to. All Criterias are associated with a single Decision."
  ...
  If your participants need to compare different aspects or an option, multiple criteria allow a participant a vote on each aspect. Participants can then (optionally) weight these aspects to state which is most important to them. By default, all criteria are weighted equally.
  """
  ...
  defp criteria_fields() do
    [
     ...
     %{name: :info, info: @info, type: "markdown", required: false, automatic: false, immutable: false},
     %{name: :bins, info: @bins, type: :integer, validation: "must be between 1 and 9", default: 5, required: true, automatic: false, immutable: false},
  ...
  """
  def fields() do
    DocsComposer.common_fields([:id, :slug, :sort, :deleted, :inserted_at, :updated_at]) ++ criteria_fields()
  end
