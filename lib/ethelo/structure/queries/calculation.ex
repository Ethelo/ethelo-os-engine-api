defmodule EtheloApi.Structure.Queries.Calculation do
  ...
  """
  def list_calculations(decision, filters \\ %{})
  def list_calculations(%Decision{} = decision, filters), do: list_calculations(decision.id, filters)
  def list_calculations(nil, _), do: raise ArgumentError, message: "you must supply a Decision"
  def list_calculations(decision_id, filters) do
    decision_id |> match_query(filters) |> preload(:variables) |> Repo.all
  end

  @doc """
  Returns a list of matching Calculations for a list of Decision ids.

  ## Examples

      iex> match_calculations(decision_id)
      [%Calculation{}, ...]

  """
  def match_calculations(filters \\ %{}, decision_ids)
  def match_calculations(filters, decision_ids) when is_list(decision_ids) do
  ...
  def match_calculations(_, nil), do: raise ArgumentError, message: "you must supply a list of Decision ids"
  ...
  def get_calculation(nil, _), do:  raise ArgumentError, message: "you must supply a Calculation id"
  def get_calculation(id, decision_id) do
    base_query() |> preload([:variables]) |> Repo.get_by(id: id, decision_id: decision_id)
  end

  @doc """
  Creates an calculation.
  ...
  """
  def create_calculation(%Decision{} = decision, %{} = attrs) do
    %Calculation{}
    |> Calculation.create_changeset(attrs, decision)
    |> Repo.insert()
  ...
  def delete_calculation(%Calculation{} = calculation, decision_id), do: delete_calculation(calculation.id, decision_id)
  def delete_calculation(_, nil), do: raise ArgumentError, message: "you must supply a Decision id"
  def delete_calculation(nil, _), do:  raise ArgumentError, message: "you must supply a Calculation id"
