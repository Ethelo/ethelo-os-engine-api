defmodule EtheloApi.Structure.Queries.Decision do
  ...
  def match_query(filters \\ %{}) do
    keywords = Map.get(filters, :keywords, nil)
    filters = filters |> Map.delete(:keywords)

    query = base_query() |> filter_query(filters, valid_filters())

    query = if( keywords == nil || keywords == [] ) do
      query
    else
  ...
  @doc """
  Gets a single Decision matching the supplied filters
  ...
  def ensure_default_associations(%Decision{} = decision) do
    ...
    Engine.Scenarios.Queries.ScenarioConfig.ensure_one_scenario_config(decision)
    {:ok, decision}
  end

  def ensure_default_associations(value), do: value

  defp current_time_hash(), do: inspect(Time.utc_now())
  ...
  def update_decision_influent_hash(%Decision{} = decision) do
    case Engine.Invocation.generate_group_influent_hash(decision) do
      {:ok, hash} ->
  ...
  def update_decision_weighting_hash(decision)

  def update_decision_weighting_hash(%Decision{} = decision) do
    case Engine.Invocation.generate_weighting_hash(decision) do
      {:ok, hash} ->
        Ecto.Changeset.change(decision, %{weighting_hash: hash})
        |> Repo.update()
  ...
  def update_decision_weighting_hash(decision_id) when is_integer(decision_id) do
    get_decision(decision_id) |> update_decision_weighting_hash()
  end
