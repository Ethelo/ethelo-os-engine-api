defmodule EtheloApi.Constraints.VariableBuilder do
  @moduledoc """
  Used to suggest possible variables based on details and filters configured on a decision.
  """
  ...
  def ensure_all_valid_variables(%Decision{} = decision) do
    ...
    suggested_variables(option_details, option_filters)
    |> create_suggested_variables(decision)

    variable_updates(option_details, option_filters)
  ...
  def valid_variables(option_details, option_filters) when is_list(option_details) and is_list(option_filters) do
    variables = detail_variables(option_details) ++ filter_variables(option_filters)
    add_ids(variables)
  end

  # variables that do not exist yet
  def suggested_variables(option_details, option_filters) when is_list(option_details) and is_list(option_filters) do
    valid_variables(option_details, option_filters)
    |> Enum.reject(&(Map.get(&1, :id, false)))
  end

  # variables that do not exist yet
  def variable_updates(%OptionDetail{} = option_detail, option_filters), do: valid_variables([option_detail], option_filters)
  def variable_updates(option_details, %OptionFilter{} = option_filter), do: valid_variables(option_details, [option_filter])
  def variable_updates(option_details, option_filters) when is_list(option_details) and is_list(option_filters) do
    valid_variables(option_details, option_filters)
    |> Enum.filter(&(Map.get(&1, :id, false)))
  end

  def update_existing_variables(updates, decision) do
    variables_by_id = Structure.list_variables(decision) |> group_by_id()

    updates |> Enum.map(fn(updated) ->

      existing = Map.get(variables_by_id, updated.id)
      attrs = Map.from_struct(updated)
        |> clean_map
        |> Map.put(:slug, nil)
        |> Map.delete(:id)
  ...
  def create_suggested_variables(variables, %Decision{} = decision) do
    ...
    |> Enum.map(&add_slug/1)
    |> Enum.map(&(Structure.create_variable(decision, &1, false)))
  end

  def add_ids([]), do: []
  def add_ids(suggested) do
    first = suggested |> hd()
    SuggestionMatcher.add_existing_ids(suggested, Structure.list_variables(first.decision_id), &matches?/2)
  end
  ...
  def matches?(suggested, existing) do
    existing.option_detail_id == suggested.option_detail_id
    and
    existing.option_filter_id == suggested.option_filter_id
    and
  ...
  def convert_to_filter_variables(%OptionFilter{} = option_filter) do
    [
      build_filter_variable(:count_selected, "Count #{option_filter.title}", "count_#{option_filter.slug}", option_filter),
    ]
  end
  ...
  def convert_to_detail_variables(%OptionDetail{format: format} = option_detail) when format in [:float, :integer] do
    [
      build_detail_variable(:sum_selected, "Total #{option_detail.title}", "total_#{option_detail.slug}", option_detail),
      build_detail_variable(:mean_selected, "Avg #{option_detail.title}", "avg_#{option_detail.slug}", option_detail),
    ]
  end
