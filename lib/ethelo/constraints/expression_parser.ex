defmodule EtheloApi.Constraints.ExpressionParser do
  @moduledoc """
  ...
  @part_types %{
    close: %{message: "braces must be matched and must contain expressions", invalid_next: [:open, :invalid,  :number]},
    open: %{message: "braces must be matched and must contain expressions", invalid_next: [:close, :invalid, :operator, :end]},
    method_open: %{message: "methods must have open and closing methods", invalid_next: [:invalid, :operator, :method_close, :close, :end]},
    method_close: %{message: "methods must have open and closing methods", invalid_next: [:invalid,  :method_open, :open, :number, :variable]},
    operator: %{message: "must have a number or variable on each side of an operator", invalid_next: [:close, :invalid, :operator, :end]},
    number: %{message: "must follow a number or variable with an operator", invalid_next: [:number, :invalid, :variable, :open]},
    variable: %{message: "must have a number or variable with an operator", invalid_next: [:number, :invalid, :variable, :open]},
  ...
  defp split_expression(expression) do
    Regex.split(~r/([\+\/\*\s\(\)\}]|[a-z0-9\.\-_]+{?)/i, expression, include_captures: true, trim: true)
  end
