defmodule EtheloApi.Constraints.FilterOptions do
  @moduledoc """
  Tools for building queries from filter configs.

  This tool loads all options and option detail values and filters them,
  ...
  @doc """
  Returns a list of matched Option ids for a single OptionFilters
  Use an optional second argument to only include enabled options.

  ## Examples

      iex> option_ids_matching_filter(filter)
      [1,45]

      iex> option_ids_matching_filter(filter, true)
      [1]

  """
  def option_ids_matching_filter(%OptionFilter{id: id} = filter, enabled_only \\ false) do
    list =
      [filter]
      |> option_ids_matching_filters(enabled_only)
      |> get_in([:ids, id])
  ...
  @doc """
  Returns the list of matched Option ids for an OptionFilter.
  Use an optional second argument to only include enabled options.
  ...
  """
  def option_ids_matching_filters(filter_list, enabled_only \\ false)
  def option_ids_matching_filters([], _), do: %{slugs: %{}, ids: %{}}

  def option_ids_matching_filters(filter_list, enabled_only) when is_list(filter_list) do
  ...
  def option_ids_matching_filter_data(%{options: _, option_filters: _} = voting_data) do
    options_by_category = options_by_category_and_all(voting_data.options)

    all_options_index =
      option_ids_matching_all_options_filter(voting_data.option_filters, options_by_category)

    category_index =
      option_ids_matching_category_filters(voting_data.option_filters, options_by_category)

    detail_index = option_ids_matching_detail_filters(voting_data)
  ...
  def option_ids_matching_detail_filters(%{option_filters: _, option_details: _} = voting_data) do
    filter_list =
      voting_data.option_filters
      |> Enum.filter(&Map.get(&1, :option_detail_id, false))

    detail_list = voting_data.option_details |> group_by_id()

    odv_list = voting_data.option_detail_values |> group_by_option_detail()
  ...
  def load_odvs_for_detail_filters(filter_list, enabled_only) do
    ...
    query =
      Option
      |> join(:left, [o], odv in OptionDetailValue, odv.option_id == o.id)
