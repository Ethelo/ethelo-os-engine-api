defmodule EtheloApi.Structure.SuggestionMatcher do
  @moduledoc """
  ...
  defp add_ids(suggested, existing, checked_suggestions, matcher) do
    ...
    {remaining_existing, checked_suggestions} =
      match_in(first, existing, [], checked_suggestions, matcher)
  ...
  defp match_in(suggested, existing, checked_records, checked_suggestions, matcher) do
    ...
    if matcher.(suggested, record) do
      {remaining_existing ++ checked_records,
       add_matched_to_checked(suggested, record, checked_suggestions)}
    else
      match_in(
        suggested,
        remaining_existing,
        [record | checked_records],
        checked_suggestions,
        matcher
      )
    end
