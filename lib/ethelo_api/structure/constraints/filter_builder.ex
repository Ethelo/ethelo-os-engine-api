defmodule EtheloApi.Structure.FilterBuilder do
  @moduledoc """
  Used to suggest possible OptionFilters based on OptionDetails configured on a Decision.
  """
  ...
  @doc """
  Populates missing OptionFilters
  ...
  def create_filters(%{missing_filters: missing, decision: decision}) when is_list(missing) do
    ...
    |> Enum.map(&Structure.create_option_filter(&1, decision, false))
    |> Enum.map(&elem(&1, 1))
  end
