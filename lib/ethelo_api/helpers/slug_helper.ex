defmodule EtheloApi.Helpers.SlugHelper do
  ...
  def changeset_decision_id(changeset) do
    decision_id = get_field(changeset, :decision_id)

    if decision_id == nil do
      get_field(changeset, :decision) |> Map.get(:id)
    else
      decision_id
    end
  end
  ...
  def slug_not_found_in_decision(queryable, slug, %Ecto.Changeset{} = changeset) do
    decision_id = changeset_decision_id(changeset)
