defmodule EtheloApi.Invocation.ScoringMapLoader do
  @moduledoc """
  Loads only required fields as maps instead of structures for efficenicy
  this is a helper utility used by `EtheloApi.Invocation.ScoringData`
  """
  ...
  def criteria_data(decision_id) do
    ...
    |> select(
      [s],
      map(s, [:id, :slug, :apply_participant_weights, :weighting, :deleted, :decision_id])
    )
    |> Repo.all()
  end
  ...
  def option_data(decision_id) do
    ...
    |> select(
      [s],
      map(s, [:id, :slug, :enabled, :determinative, :deleted, :option_category_id, :decision_id])
    )
    |> Repo.all()
  end
  ...
  def scenario_config_data(decision_id) do
    EtheloApi.Structure.ScenarioConfig
    |> where([s], s.decision_id == ^decision_id)
    |> Repo.all()
  end
  ...
  def option_category_weight_data(decision_id, participant_id \\ nil) do
    query =
      EtheloApi.Voting.OptionCategoryWeight
  ...
  def criteria_weight_data(decision_id, participant_id \\ nil) do
    query =
      EtheloApi.Voting.CriteriaWeight
