defmodule EtheloApi.Invocation.OptionBuilder do
  @moduledoc """
  Convert Options and their associated OptionDetailValues to invocation json
  Also creates OptionDetailValues based on inclusion in OptionFilter groups
  """

  import EtheloApi.Invocation.Slugger
  ...
  defp category_details(option, option_categories) do
    option_categories
  ...
  defp value_details(odvs, option_details) when is_list(odvs) do
  ...
  defp value_details(odv, option_details) do
  ...
  defp filter_details(%{} = option, %{} = options_by_filter) do
