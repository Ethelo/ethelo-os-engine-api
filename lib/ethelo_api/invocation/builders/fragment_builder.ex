defmodule EtheloApi.Invocation.FragmentBuilder do
  @moduledoc """
  Convert Variables into reusuable fragments that will be applied to
  Constraints and ScenarioDisplays
  """
  ...
  defp loop_code(loop_method, variable, details) do
    details
  ...
  defp sum_selected_code(variable, details) do
    details
    |> Map.get(variable.option_detail_id)
    |> detail_value_slug()
    |> (fn slug -> "$#{slug}" end).()
  ...
  defp count_all_code(variable, filter_options) do
    filter_options
  ...
  defp count_selected_code(variable, filters) do
    filters
    |> Map.get(variable.option_filter_id)
    |> filter_group_slug()
    |> (fn slug -> "$#{slug}" end).()
