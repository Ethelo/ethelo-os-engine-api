defmodule EtheloApi.Voting.VoteExportData do
  ...
  # export_path = "votes_#{decision_id}.json"
  # vote_dump = EtheloApi.Voting.VoteExportData.vote_data_dump(decision_id)
  # {:ok, json} = Jason.encode(vote_dump)
  #  File.write(export_path, json)
  #  out of iex - docker cp a10531ab1ef9:/ethelodockerstack/votes_879.json /sassadmin/tmp
  ...
  def vote_data_dump(decision_id) do
    ...
    %{
      ...
      option_category_range_votes:
        context.option_category_range_votes
        |> Enum.map(fn bin_vote -> build_vote_data(bin_vote, context) end),
      option_category_weights:
        context.option_category_weights
        |> Enum.map(fn bin_vote -> build_vote_data(bin_vote, context) end),
      criteria_weights:
        context.criteria_weights
        |> Enum.map(fn bin_vote -> build_vote_data(bin_vote, context) end)
    }
  end

  def to_json(result, pretty), do: Jason.encode!(result, pretty: pretty)

  def build_vote_data(record, context) do
    record =
      case Map.get(record, :option_id, nil) do
        nil -> record
        option_id -> Map.put(record, :option_title, get_in(context.options, [option_id, :title]))
      end

    record =
      case Map.get(record, :option_category_id, nil) do
        nil ->
          record
        ...
        option_category_id ->
          Map.put(
            ...
            get_in(context.option_categories, [option_category_id, :title])
          )
      end

    record =
      case Map.get(record, :criteria_id, nil) do
        nil ->
          record

        criteria_id ->
          Map.put(record, :criteria_title, get_in(context.criterias, [criteria_id, :title]))
      end
    ...
    record =
      Map.put(
        record,
        :participant_id,
        get_in(context.participants, [Map.get(record, :participant_id), :id])
      )
