defmodule EtheloApi.Scenarios.Calculators.HistogramCalculator do
  use Ecto.Schema
  alias EtheloApi.Invocation.ScoringData
  alias EtheloApi.Invocation.InfluentsJson
  import EtheloApi.Helpers.ExportHelper

  def bin_vote_histogram(%{scenario_config: config} = import_data, %{
        option: option,
        criteria: criteria
      }) do
    import_data.bin_votes_by_option
    |> Map.get(option.id, [])
    |> Enum.filter(fn bin_vote -> bin_vote.criteria_id == criteria.id end)
    |> convert_to_bin_histogram(config.bins)
  end

  def bin_vote_histogram(%{scenario_config: config} = import_data, %{option: option}) do
    option_category = import_data.option_categories_by_id[option.option_category_id]
  ...
  def option_range_vote_counts(%{} = import_data, %{option: option}) do
    category_votes =
      import_data.option_category_range_votes_by_oc
