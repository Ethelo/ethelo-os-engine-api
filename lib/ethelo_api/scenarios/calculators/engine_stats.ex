defmodule EtheloApi.Scenarios.Calculators.EngineStats do
  @moduledoc """
  Parses Engine output into expected stat hashes, including filling in association ids
  """
  ...
  def extract_option_stats(import_data, %{} = engine_scenario) do
    option_data = get_in(engine_scenario, ["stats", "options"]) || []

    Enum.map(option_data, fn {option_slug, engine_stats} ->
      option = Map.get(import_data.options_by_slug, option_slug, %{slug: option_slug})
      %{option: option} |> Map.merge(prepare_engine_stats(engine_stats))
    end)
  end

  def extract_criteria_stats(import_data, %{} = engine_scenario) do
    ...
    Enum.map(criteria_option_data, fn {option_slug, criteria_data} ->
      ...
      Enum.map(criteria_data, fn {criteria_slug, engine_stats} ->
        criteria = Map.get(import_data.criterias_by_slug, criteria_slug, %{slug: criteria_slug})
  ...
  def extract_option_category_stats(import_data, %{} = engine_scenario) do
    ...
    Enum.map(option_category_data, fn {option_category_slug, engine_stats} ->
      option_category =
        Map.get(
          import_data.option_categories_by_slug,
          String.slice(option_category_slug, 1..-1//1),
          %{slug: option_category_slug}
        )
